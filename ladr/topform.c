...
{
  if (c->is_formula)
    gather_symbols_in_formula(c->formula, rcounts, fcounts);
  else {
    Literals lit;
    for (lit = c->literals; lit; lit = lit->next)
      gather_symbols_in_formula_term(lit->atom, rcounts, fcounts);
  }
}  /* gather_symbols_in_topform */
...
void gather_symbols_in_topforms(Plist lst, int *rcounts, int *fcounts)
{
  ...
  for (p = lst; p; p = p->next)
    gather_symbols_in_topform(p->v, rcounts, fcounts);
}  /* gather_symbols_in_topforms */
