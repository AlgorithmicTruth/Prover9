...
Pair_index init_pair_index(int n)
{
  ...
  p->lists = safe_malloc(n * sizeof(Clist));
  p->top   = safe_malloc(n * n * sizeof(Clist_pos));
  p->curr  = safe_malloc(n * n * sizeof(Clist_pos));
...
void p_pair_index(Pair_index p)
{
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      printf(" [");
      if (p->top[i*n+j])
	printf("%2llu",p->top[i*n+j]->c->id);
      else
	printf("--");
      printf(",");
      if (p->curr[i*n+j])
	printf("%2llu",p->curr[i*n+j]->c->id);
      else
	printf("--");
      printf("] ");
      fflush(stdout);
