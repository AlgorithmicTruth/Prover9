...
{
  /* The original recursion processes the list right-to-left: it recurses
     to the end, then on the way back builds binary terms from right to
     left.  We replicate this by first collecting matching attributes
     into a temporary array, then building the result from last to first. */
  Attribute matches[MAX_ATTRIBUTE_NAMES * 4];  /* generous upper bound */
  int count = 0;
  Term t;
  int i;
  ...
  /* Build from rightmost match to leftmost, exactly as the recursion did. */
  t = NULL;
  for (i = count - 1; i >= 0; i--) {
    Term head = copy_term(matches[i]->u.t);
    if (t == NULL)
      t = head;
    else
...
Attribute delete_attributes(Attribute a, int id)
{
  ...
  while (*p != NULL) {
    if ((*p)->id == id) {
      ...
      *p = victim->next;
      /* If there is any memory associted with the attribure, free it here. */
      if (attribute_type(victim) == TERM_ATTRIBUTE)
	zap_term(victim->u.t);
      free_attribute(victim);
    }
    else {
      p = &(*p)->next;
    }
...
{
  /* Build a right-associated binary tree of attribute terms.
     Collect nodes into an array, then build from right to left. */
  Attribute nodes[MAX_ATTRIBUTE_NAMES * 4];  /* generous upper bound */
  int count = 0;
  int sn, i;
  Term t;
  Attribute cur;
  ...
  if (count == 1)
    return build_attr_term(nodes[0]);

  sn = str_to_sn(operator, 2);
  t = build_attr_term(nodes[count - 1]);
  for (i = count - 2; i >= 0; i--)
    t = build_binary_term(sn, build_attr_term(nodes[i]), t);
  return t;
}  /* attributes_to_term */
...
Attribute cat_attributes(Attribute a0, Attribute a1)
{
  ...
  if (a0 == NULL)
    return a1;
  for (p = &a0; *p != NULL; p = &(*p)->next)
    ;  /* walk to end of list a0 */
  *p = a1;
  return a0;
}  /* cat_attributes */
