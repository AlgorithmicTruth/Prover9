...
{
  /* Iterative term-tree canonicalization using explicit stack. */
  ...
  while (top >= 0) {
    ...
    if (term_flag(cur, flag) || VARIABLE(cur)) {
      /* Already done or variable -- pop. */
      top--;
      continue;
    }

    /* Phase 1: push children that still need processing. */
    if (stack[top].child < ARITY(cur)) {
      int c = stack[top].child;
      stack[top].child++;
      Term ch = ARG(cur, c);
    ...
    /* Phase 2: all children canonicalized -- do AC/C swapping. */
    restart = FALSE;
    {
      int sn = SYMNUM(cur);
      if (ilist_member(AC_symbols, sn)) {
        Term a = ARG(cur,0);
        Term b = ARG(cur,1);
        if (same_top(cur, b) && can_compare(a, ARG(b,0), sn) == GREATER_THAN) {
          Term b0 = ARG(b,0);
          ARG(cur,0) = b0;
          ARG(b,0) = a;
          term_flag_clear(b, flag);
          /* Need to re-canonicalize b, then re-canonicalize cur. */
          stack[top].child = 0;  /* restart cur */
          restart = TRUE;
        }
    ...
    if (!restart) {
      term_flag_set(cur, flag);  /* mark as canonicalized */
      top--;
    }
  }
  return t;
...
BOOL cac_tautology(Literals lits)
{
  ...
  if (C_symbols == NULL)
    return FALSE;
  for (p = lits; p != NULL; p = p->next) {
    if (p->sign && cac_redundant_atom(p->atom))
      return TRUE;
  }
  return FALSE;
}  /* cac_tautology */
