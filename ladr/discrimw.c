...
int num_ac_args(struct term *t, int symbol)
{
  ...
  while (top > 0) {
    Term cur = stack[--top];
    if (!COMPLEX(cur) || SYMNUM(cur) != symbol)
      count++;
    else {
...
void print_discrim_wild_tree(FILE *fp, Discrim d, int n, int depth)
{
  ...
  while (top > 0) {
    ...
    if (cur_depth == 0)
      fprintf(fp, "\nroot");
    else if (cur->type == AC_ARG_TYPE)
      fprintf(fp, "AC %d args", cur->symbol);
    else if (cur->type == AC_NV_ARG_TYPE)
      fprintf(fp, "AC %d NV args", cur->symbol);
    else if (DVAR(cur))
      fprintf(fp, "*");
    else
...
Discrim discrim_wild_insert_rec(Term t, Discrim d)
{
  /*
   * Iterative version using heap-allocated dynamically-grown stack.
   * Traverses the term in prefix order, finding/creating discrim nodes.
   * The discrim node chains forward through each symbol in sequence.
				    ...
				    Plist *path_p)
{
  /*
   * Iterative version using heap-allocated dynamically-grown stack.
   * Traverses the term in prefix order, looking for existing discrim nodes.
  ...
  while (ttop > 0) {
    ...
    *path_p = p;
	...
	na = num_ac_args(t, SYMNUM(t));
	nnv = num_ac_nv_args(t, SYMNUM(t));
...
void discrim_wild_delete(Term t, Discrim root, void *object)
{
  ...
  if (!end->u.data) {
	...
	i3->next = i2->next;
       ...
       */
      if (parent->kid_hash != NULL && !DVAR(i2)) {
	discrim_ht_delete(parent->kid_hash, i2->symbol);
