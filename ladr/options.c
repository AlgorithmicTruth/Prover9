		    ...
		    ...)
{
  ...
  if (Next_stringparm == MAX_STRINGPARMS)
    ...
    Stringparms[id].range = safe_malloc(n * sizeof(char *));
    for (i = 0; i < n; i++)
...
/*************
 *
 *    fwrite_options_input()
 *
 *************/

/* DOCUMENTATION
Write the current option values in parseable LADR input format
(one command per line, no banner).  Suitable for saved_input.txt
in checkpoints.
*/

/* PUBLIC */
void fwrite_options_input(FILE *fp)
{
  int i;
  ...
  for (i = 0; i < Next_flag; i++) {
    ...
    if (str_ident(Flags[i].name, "ignore_option_dependencies"))
      continue;
    fprintf(fp, "%s(%s).\n", Flags[i].val ? "set" : "clear", Flags[i].name);
  }

  for (i = 0; i < Next_parm; i++) {
    fprintf(fp, "assign(%s, %d).\n", Parms[i].name, Parms[i].val);
  }

  for (i = 0; i < Next_floatparm; i++) {
    fprintf(fp, "assign(%s, \"%.3f\").\n",
            Floatparms[i].name, Floatparms[i].val);
  }

  for (i = 0; i < Next_stringparm; i++) {
    fprintf(fp, "assign(%s, %s).\n",
            Stringparms[i].name, Stringparms[i].val);
  }
  fflush(fp);
}  /* fwrite_options_input */
...
void assign_parm(int id, int val, BOOL echo)
{
  if (id < 0 || id >= Next_parm) {
    ...
    Parms[id].val = val;
    Option_updates++;
    /* Push max_megs to memory system immediately so it takes effect
       during input parsing, not just after input is fully read. */
    if (str_ident(Parms[id].name, "max_megs"))
      set_max_megs(val);
    /* Push report_preprocessing to top_input immediately so it takes
       effect before clausification begins. */
    if (str_ident(Parms[id].name, "report_preprocessing"))
      set_preprocessing_report_interval(val);
    if (!Ignore_dependencies) {
