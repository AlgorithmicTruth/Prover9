...
struct symbol {
  ...
  int         name_len;         /* strlen(name), for fast comparison */
...
{
  Symbol p = safe_malloc(sizeof(struct symbol));
...
static
unsigned hash_sym(char *s, int arity, int *len_out)
{
  unsigned x = arity;
  char *start = s;
  while (*s != '\0') {
...
static
Symbol lookup_by_sym(char *name, int arity, int *hashval_out, int *len_out)
{
  int name_len;
  unsigned h = hash_sym(name, arity, &name_len);
  Plist p;
  *hashval_out = h;
  *len_out = name_len;
  for (p = By_sym[h]; p; p = p->next) {
    Symbol s = p->v;
    if (s->arity == arity && s->name_len == name_len &&
...
{
  int hashval_sym, len;
  Symbol s = lookup_by_sym(str, arity, &hashval_sym, &len);
...
void clear_parse_type(char *str)
{
  int h, n;
  Symbol p;
  p = lookup_by_sym(str, 1, &h, &n);
  if (p != NULL) {
    p->parse_type = NOTHING_SPECIAL;
    p->parse_prec = 0;
  }
  p = lookup_by_sym(str, 2, &h, &n);
  if (p != NULL) {
    p->parse_type = NOTHING_SPECIAL;
    p->parse_prec = 0;
  }
...
{
  int h, n;
  Symbol p = lookup_by_sym(str, 2, &h, &n);
...
{
  int h, n;
  Symbol p = lookup_by_sym(str, 1, &h, &n);
  if (p == NULL || p->parse_type == NOTHING_SPECIAL)
    return FALSE;
  else {
    *precedence = p->parse_prec;
...
{
  (void) str_to_sn(false_sym(), 0);
  (void) str_to_sn(true_sym(), 0);
  (void) str_to_sn("false", 0);
  (void) str_to_sn("true", 0);
}  /* declare_base_symbols */
...
{
  /* Iterative: build new list of variable symbols, preserving order. */
  Ilist result = NULL;
  Ilist tail = NULL;
  for (; syms != NULL; syms = syms->next) {
    if (sn_to_arity(syms->i) == 0 && variable_name(sn_to_str(syms->i))) {
      Ilist node = ilist_prepend(NULL, syms->i);
      if (tail == NULL)
	result = node;
      else
	tail->next = node;
...
Ilist all_function_symbols(void)
{
  ...
  for (i = 0; i < SYM_TAB_SIZE; i++) {
	...
	syms = ilist_prepend(syms, s->symnum);
    }
  }
  return syms;
}  /* all_function_symbols */
...
BOOL skolem_ok(char *name, int arity)
{
  if (!Skolem_check)
    ...
    int h, n;
    Symbol s = lookup_by_sym(name, arity, &h, &n);
    if (s == NULL)
      return TRUE;
    else
      return s->auxiliary;
	       ...
	       Ordertype (*comp_proc) (Symbol, Symbol))
{
  ...
  Symbol *a = safe_malloc(n * sizeof(void *));
  Ilist p;
  int i = 0;
...
static int compare_lex_val(const void *a, const void *b)
{
  int sa = *(const int *)a;
  int sb = *(const int *)b;
  int va = sn_to_lex_val(sa);
  int vb = sn_to_lex_val(sb);
  if (va < vb) return -1;
  if (va > vb) return 1;
  return 0;
}
...
Ilist sort_by_lex_val(Ilist p)
{
  ...
  qsort(arr, n, sizeof(int), compare_lex_val);

  /* Reuse existing Ilist nodes */
  result = p;
...
Ilist not_in_preliminary_precedence(Ilist syms, Symbol_type type)
{
  ...
  for (p = syms; p; p = p->next) {
	...
	position_in_ilist(p->i, Preliminary_prec_func) == -1)
      missing = ilist_prepend(missing, p->i);

    else if (type == PREDICATE_SYMBOL &&
	     position_in_ilist(p->i, Preliminary_prec_pred) == -1)
      missing = ilist_prepend(missing, p->i);
  }
  return reverse_ilist(missing);
}  /* not_in_preliminary_precedence */
...
{
  /* Iterative: insert i before the first node with higher arity. */
  int target_arity = sn_to_arity(i);
  if (prec == NULL)
    return ilist_append(NULL, i);
  if (sn_to_arity(prec->i) > target_arity)
    return ilist_prepend(prec, i);
  {
    Ilist p = prec;
    while (p->next != NULL && sn_to_arity(p->next->i) <= target_arity)
      p = p->next;
    /* Insert after p */
    p->next = ilist_prepend(p->next, i);
    return prec;
  }
}  /* skolem_insert */
...
int symbol_with_string(Ilist syms, char *str)
{
  ...
  while (syms != NULL) {
    if (str_ident(str, sn_to_str(syms->i)))
      return syms->i;
    syms = syms->next;
  }
  return -1;
...
/*************
 *
 *   set_preliminary_precedence_ilist()
 *
 *   Set the preliminary (user-specified) symbol precedence directly
 *   from a list of symnums.  Used by checkpoint resume to restore
 *   the exact symbol ordering from the original run.
 *
 *************/

/* PUBLIC */
void set_preliminary_precedence_ilist(Ilist symnums, Symbol_type type)
{
  if (type == FUNCTION_SYMBOL)
    Preliminary_prec_func = symnums;
  else
    Preliminary_prec_pred = symnums;
}  /* set_preliminary_precedence_ilist */

/*************
 *
 *   get_preliminary_precedence_ilist()
 *
 *   Return a COPY of the preliminary precedence Ilist for the given type.
 *   Returns NULL if no preliminary precedence exists.
 *   Caller must zap_ilist the result.
 *
 *************/

/* PUBLIC */
Ilist get_preliminary_precedence_ilist(Symbol_type type)
{
  Ilist src = (type == FUNCTION_SYMBOL) ? Preliminary_prec_func
                                        : Preliminary_prec_pred;
  return copy_ilist(src);
}  /* get_preliminary_precedence_ilist */
...
/* PUBLIC */
Ilist symnums_of_arity(Ilist p, int arity)
{
  /* Iterative: filter in-place, freeing non-matching nodes. */
  /* Skip and free leading non-matching nodes */
  while (p != NULL && sn_to_arity(p->i) != arity) {
    Ilist r = p->next;
    free_ilist(p);
    p = r;
  }
