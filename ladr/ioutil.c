...
extern double preprocessing_report_starting(void);
 ...
 *   sb_write_term_tptp()
 *
 *   Write a term to a String_buf, replacing LADR "-" negation with "~".
 *
 *************/

static
void sb_write_term_tptp(String_buf sb, Term t)
{
  String_buf tmp = get_string_buf();
  char *s;
  int i;

  sb_write_term(tmp, t);
  s = sb_to_malloc_string(tmp);
  zap_string_buf(tmp);
...
/*************
 *
 *   sb_write_formula_tptp()
 *
 *   Recursively write a Formula in TPTP syntax to a String_buf.
 *
 *************/

static
void sb_write_formula_tptp(String_buf sb, Formula f)
{
  if (f->type == ATOM_FORM) {
    sb_write_term_tptp(sb, f->atom);
  }
  else if (f->type == NOT_FORM) {
    sb_append(sb, "~ (");
    sb_write_formula_tptp(sb, f->kids[0]);
    sb_append(sb, ")");
  }
    ...
    if (f->arity == 0)
      ...
      for (i = 0; i < f->arity; i++) {
        if (i > 0) sb_append(sb, " & ");
        sb_write_formula_tptp(sb, f->kids[i]);
      }
      sb_append(sb, ")");
    }
  }
    ...
    sb_write_formula_tptp(sb, f->kids[0]);
    sb_append(sb, " => ");
    sb_write_formula_tptp(sb, f->kids[1]);
    sb_append(sb, ")");
  }
    ...
    while (body->type == ALL_FORM) {
      ...
      sb_append(sb, body->qvar);
      first = FALSE;
      body = body->kids[0];
    }
    sb_append(sb, "] : (");
    sb_write_formula_tptp(sb, body);
    sb_append(sb, ")");
  }
...
/*************
 *
 *   sb_tstp_write_clause_jmap()
 *
 *   Write one clause in TSTP annotated formula format to a String_buf.
 *   Format:  cnf(c_ID, role, (literals), source).
 *
 *   Uses the clause ID table (find_clause_by_id) for FOF flattening.
 *   Callers must register clauses with register_clause_with_id() first.
 *
 *************/

/* PUBLIC */
void sb_tstp_write_clause_jmap(String_buf sb, Topform c, I3list map)
{
  ...
  /* FOF flattening: skip FOF input/goal entries */
  if (is_fof &&
      (primary_type == INPUT_JUST || primary_type == GOAL_JUST))
    return;

  /* Check if clausified from a skipped FOF axiom */
  if (primary_type == CLAUSIFY_JUST) {
  ...
  /* Check if denial of a skipped FOF conjecture */
  if (primary_type == DENY_JUST) {
    Ilist parents = get_parents(just, FALSE);
    if (parents) {
      Topform parent = find_clause_by_id(parents->i);
      if (parent && parent->is_formula &&
  ...
  /* cnf/fof(c_ID, role, */
  sprintf(id_buf, "%llu", c->id);
  sb_append(sb, is_fof ? "fof(c_" : "cnf(c_");
  sb_append(sb, id_buf);
  sb_append(sb, ", ");
  ...
  if (primary_type == INPUT_JUST || promote_to_axiom)
    ...
    sb_append(sb, "negated_conjecture, ");
  else
    sb_append(sb, "plain, ");

  /* formula/clause body */
  if (is_fof && c->formula != NULL) {
    sb_write_formula_tptp(sb, c->formula);
  }
  else {
  ...
  if (primary_type == INPUT_JUST || promote_to_axiom ||
    ...
    sb_append(sb, ",\n    inference(");
    sb_append(sb, (char *) rule);
    sb_append(sb, ", [status(thm)], [");

    parents = get_parents(just, TRUE);
    first = TRUE;
    for (p = parents; p; p = p->next) {
      if (!first)
        sb_append(sb, ", ");
      sprintf(id_buf, "c_%d", p->i);
      sb_append(sb, id_buf);
      first = FALSE;
    }
    zap_ilist(parents);
			...
			
{
  if (c == NULL)
    ...
    if (format == CL_FORM_XML)
      ...
      sb_tagged_write_clause_jmap(sb, c, map);
    else if (format == CL_FORM_TSTP)
      sb_tstp_write_clause_jmap(sb, c, map);
    else
...
Plist read_term_list(FILE *fin, FILE *fout)
{
  ...
  while (t != NULL && !end_of_list_term(t)) {
    p = plist_prepend(p, t);
    rp_count++;
    preprocessing_report_check("read", rp_count, -1, -1, &rp_next);
    t = read_term(fin, fout);
  }
  if (t != NULL)
    zap_term(t);
