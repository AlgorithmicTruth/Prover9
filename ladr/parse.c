...
/*
 * Parse-type cache, indexed by symnum of arity-0 Pterms.
 * Avoids repeated hash table lookups (lookup_by_sym, lookup_by_id).
 * Each entry is a uint32_t with the following bit layout:
 *   Bit 0:     known NOT a unary parse type (negative cache)
 *   Bit 1:     known NOT a binary parse type (negative cache)
 *   Bit 2:     HAS cached unary result (positive cache)
 *   Bits 3-5:  unary Parsetype (3 bits, values 0-7)
 *   Bits 6-15: unary precedence (10 bits, values 0-1023)
 *   Bit 16:    HAS cached binary result (positive cache)
 *   Bits 17-19: binary Parsetype (3 bits, values 0-7)
 *   Bits 20-29: binary precedence (10 bits, values 0-1023)
 * Cleared by invalidate_parse_type_cache() when parse types change.
 */

#define PT_CACHE_SIZE 256
static uint32_t Pt_cache[PT_CACHE_SIZE];

/* Bit positions */
#define PT_NEG_UNARY    0x01
#define PT_NEG_BINARY   0x02
#define PT_POS_UNARY    0x04
#define PT_UNARY_TYPE_SHIFT   3
#define PT_UNARY_PREC_SHIFT   6
#define PT_POS_BINARY   0x10000
#define PT_BINARY_TYPE_SHIFT  17
#define PT_BINARY_PREC_SHIFT  20

static inline uint32_t pt_encode_unary(int prec, Parsetype type)
{
  return PT_POS_UNARY
    | ((uint32_t)type << PT_UNARY_TYPE_SHIFT)
    | ((uint32_t)prec << PT_UNARY_PREC_SHIFT);
}

static inline uint32_t pt_encode_binary(int prec, Parsetype type)
{
  return PT_POS_BINARY
    | ((uint32_t)type << PT_BINARY_TYPE_SHIFT)
    | ((uint32_t)prec << PT_BINARY_PREC_SHIFT);
}

/* PUBLIC */
void invalidate_parse_type_cache(void)
{
  int i;
  for (i = 0; i < PT_CACHE_SIZE; i++)
    Pt_cache[i] = 0;
}  /* invalidate_parse_type_cache */
  ...
  if (start == end) {
    if (is_term(start->t, ",", 0))
	 ...
	 where - is prefix and v is infix.
       ...
       */
      int sn_b = SYMNUM(start->t);
      BOOL is_unary;
      if (sn_b < PT_CACHE_SIZE && (Pt_cache[sn_b] & PT_POS_UNARY))
	is_unary = TRUE;
      else if (sn_b < PT_CACHE_SIZE && (Pt_cache[sn_b] & PT_NEG_UNARY))
	is_unary = FALSE;
      else
	is_unary = (special_parse_type(sn_to_str(sn_b)) == 1);
      if (is_unary)
	return NULL;
      else
    ...
    if (ARITY(start->t) == 0 && !start->parenthesized) {
      int sn_s = SYMNUM(start->t);
      uint32_t cache_s = (sn_s < PT_CACHE_SIZE) ? Pt_cache[sn_s] : 0;
      if (cache_s & PT_POS_UNARY) {
	prec = (cache_s >> PT_UNARY_PREC_SHIFT) & 0x3FF;
	type = (Parsetype)((cache_s >> PT_UNARY_TYPE_SHIFT) & 0x07);
	rc = 1;
	str = sn_to_str(sn_s);
      }
      else if (cache_s & PT_NEG_UNARY) {
	rc = 0;
      }
      else {
	str = sn_to_str(sn_s);
	rc = unary_parse_type(str, &prec, &type);
	if (sn_s < PT_CACHE_SIZE) {
	  if (rc)
	    Pt_cache[sn_s] |= pt_encode_unary(prec, type);
	  else
	    Pt_cache[sn_s] |= PT_NEG_UNARY;
	}
      }
      if (rc && prec <= m &&
    ...
    if (ARITY(end->t) == 0 && !end->parenthesized) {
      int sn_e = SYMNUM(end->t);
      uint32_t cache_e = (sn_e < PT_CACHE_SIZE) ? Pt_cache[sn_e] : 0;
      if (cache_e & PT_POS_UNARY) {
	prec = (cache_e >> PT_UNARY_PREC_SHIFT) & 0x3FF;
	type = (Parsetype)((cache_e >> PT_UNARY_TYPE_SHIFT) & 0x07);
	rc = 1;
	str = sn_to_str(sn_e);
      }
      else if (cache_e & PT_NEG_UNARY) {
	rc = 0;
      }
      else {
	str = sn_to_str(sn_e);
	rc = unary_parse_type(str, &prec, &type);
	if (sn_e < PT_CACHE_SIZE) {
	  if (rc)
	    Pt_cache[sn_e] |= pt_encode_unary(prec, type);
	  else
	    Pt_cache[sn_e] |= PT_NEG_UNARY;
	}
      }
      if (rc && prec <= m &&
	...
	int sn_n = SYMNUM(start->next->t);
	uint32_t cache_n = (sn_n < PT_CACHE_SIZE) ? Pt_cache[sn_n] : 0;
	if (cache_n & PT_POS_BINARY) {
	  type = (Parsetype)((cache_n >> PT_BINARY_TYPE_SHIFT) & 0x07);
	  backward = (type == INFIX_LEFT);
	}
	else if (!(cache_n & PT_NEG_BINARY)) {
	  str = sn_to_str(sn_n);
	  rc = binary_parse_type(str, &prec, &type);
	  if (sn_n < PT_CACHE_SIZE) {
	    if (rc)
	      Pt_cache[sn_n] |= pt_encode_binary(prec, type);
	    else
	      Pt_cache[sn_n] |= PT_NEG_BINARY;
	  }
	...
	if (ARITY(op->t) == 0) {
	  int sn_o = SYMNUM(op->t);
	  uint32_t cache_o = (sn_o < PT_CACHE_SIZE) ? Pt_cache[sn_o] : 0;
	  if (cache_o & PT_POS_BINARY) {
	    prec = (cache_o >> PT_BINARY_PREC_SHIFT) & 0x3FF;
	    type = (Parsetype)((cache_o >> PT_BINARY_TYPE_SHIFT) & 0x07);
	    rc = 1;
	    str = sn_to_str(sn_o);
	  }
	  else if (cache_o & PT_NEG_BINARY) {
	    rc = 0;
	  }
	  else {
	    str = sn_to_str(sn_o);
	    rc = binary_parse_type(str, &prec, &type);
	    if (sn_o < PT_CACHE_SIZE) {
	      if (rc)
		Pt_cache[sn_o] |= pt_encode_binary(prec, type);
	      else
		Pt_cache[sn_o] |= PT_NEG_BINARY;
	    }
	  }
...
Pterm toks_to_terms(Tok_pos p)
{
  ...
  while (!done && !error) {
	...
	p->tok->type == TOK_STRING ) {
      char stack_buf[256];
      int len = sb_copy_to(p->tok->sb, stack_buf, sizeof(stack_buf));
      if (len >= 0) {
	t = get_rigid_term(stack_buf, 0);
      }
      else {
...
{
  Plist p = NULL;
  char *work = safe_malloc(strlen(onlys)+1);
