...
struct mindex_pos {
  ...
  /* Candidate limit tracking */
  int  candidate_count;  /* candidates examined in this query */
  BOOL warned;           /* already warned for this query */
...
/* Candidate limit tracking */
static int Candidate_warn_limit = -1;
static int Candidate_hard_limit = -1;
static unsigned long long Queries_warned = 0;
static unsigned long long Queries_skipped = 0;
...
Term mindex_retrieve_next(Mindex_pos pos)
{
  if (pos->index->unif_type == BACKTRACK_UNIF)
    ...
    while (tf && !ok) {
      ...
      /* Check warn limit (once per query) */
      if (Candidate_warn_limit > 0 &&
          pos->candidate_count == Candidate_warn_limit &&
          !pos->warned) {
        pos->warned = TRUE;
        Queries_warned++;
        fprintf(stderr, "WARNING: query exceeded %d candidates\n",
                Candidate_warn_limit);
      }
...
/*************
 *
 *   set_candidate_limits()
 *
 *************/

/* DOCUMENTATION
Set the candidate limits for index queries.
warn_limit: log warning when exceeded (-1 = disabled)
hard_limit: skip query when exceeded (-1 = disabled)
*/

/* PUBLIC */
void set_candidate_limits(int warn_limit, int hard_limit)
{
  Candidate_warn_limit = warn_limit;
  Candidate_hard_limit = hard_limit;
}  /* set_candidate_limits */

/*************
 *
 *   mindex_queries_warned()
 *
 *************/

/* DOCUMENTATION
Return the number of queries that exceeded the warn limit.
*/

/* PUBLIC */
unsigned long long mindex_queries_warned(void)
{
  return Queries_warned;
}  /* mindex_queries_warned */

/*************
 *
 *   mindex_queries_skipped()
 *
 *************/

/* DOCUMENTATION
Return the number of queries that were skipped due to exceeding the hard limit.
*/

/* PUBLIC */
unsigned long long mindex_queries_skipped(void)
{
  return Queries_skipped;
}  /* mindex_queries_skipped */
