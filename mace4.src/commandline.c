...
void command_line_flag(int id, char *optarg)
{
  if (!optarg || str_ident(optarg, "1")) {
    set_flag(id, !Mace4_tptp_mode);
    if (!Mace4_tptp_mode)
      printf("\n%% From the command line: set(%s).\n", flag_id_to_str(id));
  }
  else if (str_ident(optarg, "0")) {
    clear_flag(id, !Mace4_tptp_mode);
    if (!Mace4_tptp_mode)
...
{
  int n = atoi(optarg);
  assign_parm(id, n, !Mace4_tptp_mode);
  if (!Mace4_tptp_mode)
...
void usage_message(FILE *fp, Mace_options opt)
{
  ...
  "  -e n : parm report_stderr (%d). Progress reports to stderr every n\n"
			       ...
			       Mace_options opt)
{
  ...
  */
  ...
  while ((c = getopt(argc, argv,
	     "n:N:m:t:s:b:O:M:p:P:v:L:G:H:I:J:K:T:R:i:q:Q:S:cf:gAr:e:")) != EOF) {
    switch (c) {
    case 'n':
    ...
    case 'S':
      ...
      break;
    case 'e':
      command_line_parm(opt->report_stderr, optarg);
      break;

    case 'c':  /* prover compatability mode */
    case 'f':  /* input files */
    case 'g':  /* tptp syntax */
    case 'A':  /* memory logging - handled in main() */
    case 'r':  /* resume from checkpoint - handled in main() */
