...
"\nprooftrans              [expand] [renumber] [striplabels] [-f <file>]\n"
  ...
  "prooftrans tstp         [expand] [renumber] [striplabels] [-f <file>]\n"
"\n";

enum {NO_TRANS, EXPAND_EQ, EXPAND, EXPAND_IVY};   /* proof transformations */

enum {ORDINARY, PARENTS_ONLY, XML, HINTS, IVY, TAGGED, TSTP};  /* output format */
...
void read_program_input(FILE *fp, int output_format)
{
  ...
  while (s && !substring("= end of input =", s)) {
	...
	process_redeclare(cmd, FALSE, stdout);
      if (output_format != XML && output_format != IVY && output_format != TSTP) {
		 ...
		 int format, I3list jmap, int number)
{
  ...
  if (format == CL_FORM_TSTP) {
    Variable_style orig = variable_style();
    fprintf(fp, "%% SZS output start CNFRefutation\n");
    set_variable_style(PROLOG_STYLE);
    for (p = proof; p; p = p->next)
      fwrite_clause_jmap(fp, p->v, format, jmap);
    set_variable_style(orig);
...
/*************
 *
 *   attr_tree_has_non_clause()
 *
 *   Search attribute subtree for label(non_clause).
 *   Attribute tree is right-associative: #(attr1, #(attr2, attr3)).
 *
 *************/

static BOOL attr_tree_has_non_clause(Term t)
{
  if (is_term(t, attrib_sym(), 2))
    return attr_tree_has_non_clause(ARG(t, 0)) ||
           attr_tree_has_non_clause(ARG(t, 1));
  return (is_term(t, "label", 1) &&
          CONSTANT(ARG(t, 0)) &&
          str_ident(sn_to_str(SYMNUM(ARG(t, 0))), "non_clause"));
}

/*************
 *
 *   term_has_non_clause_label()
 *
 *************/

static BOOL term_has_non_clause_label(Term t)
{
  if (is_term(t, attrib_sym(), 2))
    return attr_tree_has_non_clause(ARG(t, 1));
  return FALSE;
}
...
int main(int argc, char **argv)
{
  ...
  int n, number_of_proofs;
  BOOL found;
  Plist p, c;
  int label_attr, answer_attr;
  FILE *fin = stdin;
  char *filename = NULL;
  char *label = NULL;
  ...
  if (string_member("tstp", argv, argc) ||
      string_member("TSTP", argv, argc)) {
    transformation = NO_TRANS;
    output_format = TSTP;
  }
  ...
  else if (output_format == TSTP)
    clause_format = CL_FORM_TSTP;
  else
  ...
  if (output_format != XML && output_format != HINTS && output_format != TSTP) {
    if (output_format == IVY)
      printf(";; ");
  ...
  while (found) {
    ...
    while (!substring("==========", s)) {  /* separator at end of proof */
      ...
      is_fof = (output_format == TSTP &&
                term_has_non_clause_label(clause_term));
      cl = is_fof ? term_to_topform(clause_term, TRUE)
                  : term_to_clause(clause_term);
